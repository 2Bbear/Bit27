...
DWORD WINAPI foo(void* p)
{
	...
	while (1)
	{
		...
		//int sizz=0;
		//int nRead = recvn(s, (char*)&sizz, sizeof(int), 0);//구조체 사이즈 받기
		//nRead = recvn(s, (char*)&sss, sizz, 0);//구조체 받기
		int n = 0;
		int nRead = recvn(s, (char*)&n, sizeof(int), 0); 
		
		nRead= recvn(s, (char*)&buf, n, 0);
		printf("%s \n", buf);
		//int nRead = recvn(s, buf, nRead, 0);//구조체 받기
		//들어온 데이터 분석
